...
class Countries extends Admin_Controller {
	...
	function add() {
		...
		if ($this->input->post('btnsubmit')) {
			...
			//$this->form_validation->set_rules('continent_id', 'Continent', 'trim|required|max_length[11]');
			$this->form_validation->set_rules('country_name', 'Country Name', 'trim|required|min_length[2]|max_length[100]|callback_check_combination_add');
			$this->form_validation->set_rules('country_capital', 'Country Capital', 'trim|required|min_length[2]|max_length[100]');
			$this->form_validation->set_rules('country_neighbours', 'Country Neighbour', 'trim|required|min_length[2]|max_length[500]');
			$this->form_validation->set_rules('country_currency', 'Country Currency', 'trim|required|min_length[2]|max_length[40]');
			...
			$this->form_validation->set_rules('country_tourist_rating', 'Country Tourist Rating', 'trim|required|min_length[1]|max_length[4]');
			$this->form_validation->set_rules('country_no_of_annual_tourists', 'Country Annual Tourists', 'trim|required|max_length[20]|numeric');
			...
			$this->form_validation->set_rules('country_best_season_visit', 'Country Best Season Visit', 'trim|required|min_length[1]|max_length[500]');
			$this->form_validation->set_rules('latitude', 'Country Latitude', 'trim|required|min_length[1]|max_length[50]');
			$this->form_validation->set_rules('longitude', 'Country longitude', 'trim|required|min_length[1]|max_length[50]');
			$this->form_validation->set_rules('country_conclusion', 'Country Conclusion', 'trim|required');
			$this->form_validation->set_rules('countrycode', 'Country Code', 'trim|required|callback_check_code_add');
	...
	function edit($id) {
		if ($this->input->post('btnsubmit')) {
			...
			//$this->form_validation->set_rules('continent_id', 'Continent', 'trim|required|max_length[11]');
			$this->form_validation->set_rules('country_name', 'Country Name', 'trim|required|min_length[2]|max_length[100]|callback_check_country');
			$this->form_validation->set_rules('country_capital', 'Country Capital', 'trim|required|min_length[2]|max_length[100]');
			$this->form_validation->set_rules('country_neighbours', 'Country Neighbour', 'trim|required|min_length[2]|max_length[500]');
			$this->form_validation->set_rules('country_currency', 'Country Currency', 'trim|required|min_length[2]|max_length[40]');
			...
			$this->form_validation->set_rules('longitude', 'Country longitude', 'trim|required|min_length[1]|max_length[50]');
			$this->form_validation->set_rules('country_conclusion', 'Country Conclusion', 'trim|required');
			$this->form_validation->set_rules('countrycode', 'Country Code', 'trim|required|callback_check_code_edit');
