...
class Itineraries extends REST_Controller
{
    ...
    public function planned_itineraries_get()
    {
			$data['countries']=$this->Itinerary_wm->getAllPublicItinerariesCountry();
			$message=array(
					'errorcode' =>1,
					'data'	=>$data
					);
		...
		public function browse_itinerary_post()
		{
				...
				$config["total_rows"] = $this->Itinerary_wm->countTotalPublicItineraries();
				...
				$data['public_itineraries']=$this->Itinerary_wm->getTotalPublicItineraries($config["per_page"],$start_row);
		...
		{
				 $data['reviews']=$this->Itinerary_wm->getItineraryInfo($trip);
				 ...
				 $this->Itinerary_wm->updateViews($data['reviews']['id']);
			...
			function loadSingleCountryItinerary($itineraryid,$trip_mode,$user_trip_name,$user_id)
			{
				  ...
				  $data['itineraryid']=$itineraryid;
					...
					$data['filestore']=$this->Itinerary_wm->getCitiesAttractions($cityfile,$itineraryid,'tbl_itineraries_cities');
					$data['userrating']=array();
					if($this->session->userdata('fuserid')!='')
					{
						$data['userrating']=$this->Itinerary_wm->getUserRating($itineraryid);
					}
					$this->load->vars($data);
		...
		function loadSearchedCityItinerary($itineraryid,$trip_mode,$user_trip_name,$user_id)
		{
			  ...
			  $data['webpage'] = 'planneditineraries';
				...
				$data['filestore']=$this->Itinerary_wm->getCitiesAttractions($cityfile,$itineraryid,'tbl_itineraries_searched_cities');
				$this->load->vars($data);
				$this->load->view('templates/innermaster');
		...
		function loadMultiCountryItinerary($itineraryid,$trip_mode,$user_trip_name,$user_id)
		{
			  $data['owner_id']=$user_id;
				...
				$data['latitude']=$cities[$firstkey][0]['citylatitude'];
				$data['longitude']=$cities[$firstkey][0]['citylongitude'];
				$data['basiccityname']=$cities[$firstkey][0]['city_name'];
				...
				$data['filestore']=$this->Itinerary_wm->getCitiesAttractionsMultiCountry($cityfile,$itineraryid);
				$data['webpage'] = 'planneditineraries';
		...
		public function loadQuestions()
		{
			...
			if($this->input->is_ajax_request())
			{
				...
				$checkiti=$this->Itinerary_wm->checkItineraryExist($iti);
				if($checkiti>0)
				{
			          ...
			          $config["base_url"] = site_url('Itineraries/loadQuestions');
			  		  $config["total_rows"] = $this->Itinerary_wm->countTotalQuestionOfItinerary($iti);
					  $config["full_tag_open"] = "<ul class='pagination-custom'>";
			          $config["full_tag_close"] = "</ul>";
			          $config["num_tag_open"] = "<li class='pagination-item-custom'>";
			          $config["num_tag_close"] = "</li>";
			          ...
			          $config['per_page'] = 10;
			          $this->pagination->initialize($config);
			          $data["questions"] = $this->Itinerary_wm->getQuestionOfItinerary($iti,$config["per_page"],$start_row);
		...
		public function itinerary_discussion($id)
		{
				...
				$data['question']=$this->Itinerary_wm->getQuestionDetails($id);
				if(count($data['question'])<1)
				{
					show_404();
				}
				$checkiti=$this->Itinerary_wm->checkItineraryExist($data['question']['itinerary_id']);
				if(count($checkiti)<1)
				{
					 show_404();
				}
				$data['comments']=$this->Itinerary_wm->getAllComments($id);
				$data['totalcomments']=$this->Itinerary_wm->countTotalComments($id);
				$this->load->vars($data);
				$this->load->view('templates/innermaster');
		}
		...
		function postComment()
		{
				if($this->input->is_ajax_request())
				{
					$data['comments']=$this->Itinerary_wm->postComment();
		...
		function deleteComment()
		{
			if($this->input->is_ajax_request())
			{
				$data['comments']=$this->Itinerary_wm->deleteComment();
				$output['body'] =$this->load->view('postComment_ajax', $data, true);
				$this->output->set_content_type('application/json')->set_output(json_encode($output));
			}
		}
		...
		function editComment()
		{
			if($this->input->is_ajax_request())
			{
				$data=$this->Itinerary_wm->editcomment();
		...
		function deleteQuestion()
		{
			if($this->input->is_ajax_request())
			{
				$data=$this->Itinerary_wm->deleteQuestion();
				echo $data;
			}
		}
		...
		function store_rating()
		{
				if($this->input->is_ajax_request())
				{
					$this->Itinerary_wm->store_rating();
				}
