...
/*
    function getAttractions_post()
    {
    	...
    	$selectcol='id,country_id';
    	$table='tbl_city_master';
    	$cities=selectcolbycondition($selectcol,$table,"id in (".$_POST['cityid'].")");
    	//print_r($cities);die;
    	if(count($cities)==1)
    	{
    		$data['country_id']=$cities[0]['country_id'];
    		$data['attractions']['cityid']=$cities[0]['id'];
    		$data['attractions']['filestore'] = getUserRecommededAttractionsForCity(md5($cities[0]['id']),$tags);
    	}
    	else
    	{
    		$i=0;
    		...
    		foreach ($cities as $key => $value) {
    			if($country_id!=$cities[$key]['country_id'])
    			{
    				$i++;
    				$j=0;
    			}
    			$data[$i]['country_id']=$country_id=$cities[$key]['country_id'];
    			$data[$i]['attractions'][$j]['cityid']=$cities[$key]['id'];
    			$data[$i]['attractions'][$j]['filestore'] = getUserRecommededAttractionsForCity(md5($cities[$key]['id']),$tags);
    			$j++;
    		}
    	}
