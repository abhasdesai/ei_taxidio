	...
	function cityAttractions_get()
	{
		...
		if($this->form_validation->run()==FALSE)
		{
			 $message['validation_error'] = array(
                'sdestination' 	=> form_error('sdestination'),
                'sdays' 		=> form_error('sdays'),
                'sstart_date' 	=> form_error('sstart_date')
            );
		}
		else
		{
			$flag=1;
			$other=1;
				$data['searchdata'] = $this->attractions_wm->getSearchedCity($searchinput);
			...
			if($other==1)
			{
			$data['othercities'] = $this->attractions_wm->getSearchedCityOther($data['searchcity'],$token);
			}
			else {
			...
			{
				$this->Attraction_fm->getAttractionsOfSelectedCity($cityfile,$token,0);
				$attractioncities = file_get_contents(FCPATH.'userfiles/search/'.$this->session->userdata('randomstring').'/'.$token.'/'.$filenm);
				$data['attractioncities']=json_decode($attractioncities,TRUE);
