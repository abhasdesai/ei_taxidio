...
class Recommendation extends REST_Controller
{
	...
	{				
				...
				$data['countrynm']=$countrynm=$this->recommendation_wm->getCountryNameFromSlug($slug);
				$data['countrynm']['countryimage']=site_url("userfiles/countries/".$countrynm['countryimage']);
				...
				$data['cityinfo']=$this->master_model->getcityinfo($attractioncountries[0]['id']);
				return $data;
	}

	function getcityinfo_post()
	{
				$message=array(
				'errorcode'=>1,
				'data' =>$this->master_model->getcityinfo($_POST['city_id'])
				);
		$this->set_response($message, REST_Controller::HTTP_OK);
	}
